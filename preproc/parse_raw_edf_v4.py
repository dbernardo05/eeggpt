...
def load_edf(edf_file):
	...
	for ch in raw_edf.ch_names:
		n_ch = ch.replace('EEG ', '').replace('-REF', '').replace('FP1', 'Fp1').replace('FP2','Fp2').replace('Z','z').replace(' ', '')
		new_ch_map[ch] = n_ch
	...
	if raw_edf.info['sfreq'] != 250:
		print(raw_edf.info['sfreq'])
		if raw_edf.info['sfreq'] > 250:
			raw_edf = raw_edf.copy().resample(sfreq=250)
		else:
			sys.exit()

	check_eeg(raw_edf, srate)
		...
		for subj, edf_files in (pbar := tqdm(subjs.items())):
			...
			for edf_file in edf_files:
				...
				for n in range(num_epochs):
					...
					# dataset_name = subj 
					dataset_name = subj.replace('_', 'XXX', 2)
